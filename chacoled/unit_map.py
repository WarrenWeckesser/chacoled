...
class UnitMap(HasTraits):
    ...
    # A list of 2-tuples of floats between 0 and 1.  The list of the first
    # elements must be nondecreasing, with points[0][0] = 0 and
    # points[-1][0] = 1.  (So there must always be at least two points in
    # the list.)
    points = List(Tuple)
    ...
    def add_point(self, point):
        ...
        x, y = point
        if x > 1 or x < 0:
            raise ValueError(("x = %f is not valid. "
                              "x must be between 0 and 1.") % x)
        if y > 1 or y < 0:
            raise ValueError(("y = %f is not valid. "
                              "y must be between 0 and 1.") % y)
    ...
    def delete_point(self, point):
        # Not sure what the API should be.  Might not even need this--just
        # work directly with the list. (But a tightly controlled delete
        # could prevent the first or last points from being deleted.)
    ...
    def invertible(self):
        ...
        To be invertible, the map must be monotonic and onto, so the y values
        at the ends must be 0 and 1 or 1 and 0.
        """
    ...
    def evaluate(self, x):
        ...
        if x < 0.0 or x > 1.0:
            raise ValueError(("x is %f, but evaluate(x) "
                              "requires 0 <= x <= 1.") % x)
    ...
    def compose(self, um):
        ...
        If this map is f and `um` is g, `compose` returns the UnitMap
        corresponding to f(g(x)).
        """
        new_points = []
...
def sqdistance_to_closest(p, p1, p2):
    """
    Compute the square of the distance from the point p to the line defined
    by (p1,p2).
    """
    pc = closest(p, p1, p2)
